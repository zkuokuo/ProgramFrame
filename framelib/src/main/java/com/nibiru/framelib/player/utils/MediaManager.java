...
public class MediaManager {
	...
	private class ShowThumbByTask extends AsyncTask<Void, Void, Bitmap> {
		...
		protected Bitmap doInBackground(Void... params) {
			...
			MLog.d("Player", "bitmap memory cache: " + (bitmap == null));
			// 后获取文件缓存
			if (bitmap == null || bitmap.isRecycled()) {
				bitmap = getBitmapFromDiskCache(path);
				MLog.d("Player", "bitmap disk cache: " + (bitmap == null));
			}
			...
			if (bitmap == null) {
				...
				if (bitmap != null) {
					...
					MLog.d("Player", "First get thum way");
				}
			}
			...
			if(bitmap == null) {
				...
				if (bitmap != null) {
					bitmap = addCashThum(path , bitmap);
					MLog.d("Player", "Second get thum way");
				}
			}
			...
			if (bitmap == null) {
				...
				if (bitmap != null) {
					bitmap = addCashThum(path , bitmap);
					MLog.d("Player", "Third get thum way(第三方)");
				}
			}
			...
			if (bitmap == null) {
				...
				if (bitmap != null) {
					bitmap = addCashThum(path , bitmap);
					MLog.d("Player", "Fourth get thum way");
				}
			}
			...
			if (bitmap == null) {
				...
				if (bitmap != null) {
					bitmap = addCashThum(path , bitmap);
					MLog.d("Player", "All ways can not get thum, use default thum!!!");
				}
			}
...
//		audio360List = getAudio360FileToSeparate();
		MLog.d("Player" , "getVideoList方法开始::" + System.currentTimeMillis());
...
//				}
		...
		MLog.d("Player" , "getVideoList方法结束::" + System.currentTimeMillis() + ",list的size： " + list.size());
		return list;
	}
	...
	public List<String> getAudio360FileToSeparate(){
		...
		if (file.exists()){
			MLog.d("audio360文件夹下的name ：" + file.getName());
			File[] files = file.listFiles();
			for(int i = 0 ; i < files.length ; i ++){
				MLog.d("audio360文件夹下的文件路径是：" + files[i].getPath().toString());
				String tempName = "/storage/emulated/0/audio360/" + files[i].getName();
				videoFilePathList.add(tempName);
				MLog.d("audio360文件夹下的文件转化成媒体库的路径是 ：" + tempName);
			}
		} else {
			MLog.d("audio360文件夹不存在，等待360本地播放器打开创建");
		}
