...
public final class CrashUtils {
    ...
    static {
        try {
            PackageInfo pi = UtilsCode.getApp()
                    .getPackageManager()
                    .getPackageInfo(UtilsCode.getApp().getPackageName(), 0);
            if (pi != null) {
    ...
    public static void init(final String crashDirPath, final OnCrashListener onCrashListener) {
        ...
        if (Environment.MEDIA_MOUNTED.equals(Environment.getExternalStorageState())
                && UtilsCode.getApp().getExternalCacheDir() != null)
            defaultDir = UtilsCode.getApp().getExternalCacheDir() + FILE_SEP + "crash" + FILE_SEP;
        else {
            defaultDir = UtilsCode.getApp().getCacheDir() + FILE_SEP + "crash" + FILE_SEP;
        }
