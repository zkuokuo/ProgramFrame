...
/**
 * 
 * Supported protocols: file, http, https, mms, mmsh and rtmp </br>
 * 
...
public class FFmpegMediaMetadataRetriever {
	 ...
	 */
	public Bitmap.Config frameBitmapConfig = Config.RGB_565;

	/*
	 * @SuppressLint("SdCardPath") private static final String LIBRARY_PATH =
	 * "/data/data/";
	 * 
	 * private static final String [] JNI_LIBRARIES = { "libavutil.so",
	 * "libswscale.so", "libavcodec.so", "libavformat.so",
	 * "libffmpeg_mediametadataretriever_jni.so" };
	 * 
	 * static { StackTraceElement[] stackTraceElements =
	 * Thread.currentThread().getStackTrace();
	 * 
	 * StringBuffer path = null; File file = null; boolean foundLibs = false;
	 * 
	 * for (int j = 0; j < stackTraceElements.length; j++) { String libraryPath
	 * = stackTraceElements[j].getClassName();
	 * 
	 * String [] packageFragments = libraryPath.trim().split("\\.");
	 * 
	 * path = new StringBuffer(LIBRARY_PATH);
	 * 
	 * for (int i = 0; i < packageFragments.length; i++) { if (i > 0) {
	 * path.append("."); }
	 * 
	 * path.append(packageFragments[i]); try { //System.load(path.toString() +
	 * "/lib/" + JNI_LIBRARIES[0]); file = new File(path.toString() + "/lib/" +
	 * JNI_LIBRARIES[0]); if (file.exists()) { path.append("/lib/"); foundLibs =
	 * true; break; } } catch (UnsatisfiedLinkError ex) { } }
	 * 
	 * if (foundLibs) { break; } }
	 * 
	 * if (!foundLibs) { Log.e(TAG, TAG +
	 * " libraries not found. Did you forget to add them to your libs folder?");
	...
	public Bitmap.Config getFrameBitmapConfig() {
		return frameBitmapConfig;
	}

	public void setFrameBitmapConfig(Bitmap.Config fRAME_BITMAP_CONFIG) {
		frameBitmapConfig = fRAME_BITMAP_CONFIG;
	}
	 ...
	 * changed since the last invocation of getMetadata. The set is built using
	 * the unfiltered notifications the native player sent to the
	 * MediaPlayerService during that period of time. If false, all the
	 * metadatas are considered.
	 * 
	 * @param apply_filter If true, once the metadata set has been built based
	 * on the value update_only, the current filter is applied.
	 * 
	 * @param reply[out] On return contains the serialized metadata. Valid only
	...
	public Bitmap getFrameAtTime(long timeUs, int option) {
		...
		// 图片质量
		bitmapOptionsCache.inPreferredConfig = frameBitmapConfig;
	 ...
	 */
	/*
	 * Do not change these option values without updating their counterparts in
	...
	public class Metadata {
		...
		// After a successful parsing, set the parcel with the serialized
		// metadata.
		...
		private boolean checkMetadataId(final String val) {
			 ...
			 * Log.e(TAG, "Invalid metadata ID " + val); return false; }
			 */
			return true;
		}
